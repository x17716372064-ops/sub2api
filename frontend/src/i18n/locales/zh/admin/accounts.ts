export default {
    accounts: {
      ...
      upstreamBilling: {
        ...
        adapter: '上游倍率接口适配',
        adapterNone: '原生 Sub2API（默认）',
        adapterNewAPI: 'New API 定价接口',
        adapterCustomJSON: '自定义 JSON 接口',
        adapterHint: '当上游不是原版 Sub2API，或原生探测显示“不支持”时，按上游实际接口配置。路径只允许访问当前上游主机。',
        path: '倍率接口路径',
        pathHint: '以 / 开头的同源路径，例如 /api/pricing。不要填写完整网址。',
        ratePath: 'JSON 倍率路径',
        ratePathHint: '使用点号分隔 JSON 字段，例如 group_ratio 或 data.rate；数组可使用数字下标。',
        group: '上游分组名',
        groupPlaceholder: '例如 codex特惠分组',
        groupHint: 'New API 定价接口必填；系统会从该分组的 group_ratio 中读取倍率。自定义 JSON 可留空，表示路径直接返回数字。',
        pathRequired: '请填写上游倍率接口路径',
        pathInvalid: '上游倍率接口路径无效：必须是当前主机的 / 路径，不能包含完整网址、..、查询参数或片段',
        ratePathRequired: '请填写有效的 JSON 倍率路径',
        groupRequired: 'New API 定价接口必须填写上游分组名'
      },
      upstreamAccountBalance: {
        title: '上游官网余额查询',
        hint: '填写上游官网、邮箱和密码后，保存账号时自动登录查询余额。仅支持 New API 兼容站点；密码仅写入服务端加密存储。',
        website: '上游网站',
        ...
        passwordKeep: '已保存，留空保持不变',
        passwordConfigured: '已保存上游登录密码（不可查看）',
        loading: '正在查询余额…',
        refresh: '刷新余额',
        refreshing: '刷新中…',
        clear: '清除配置',
        clearConfirm: '确定清除该账号的上游余额查询配置和已保存密码吗？',
        cleared: '上游余额查询配置已清除',
        clearFailed: '清除上游余额查询配置失败',
        balance: '余额',
        remainingQuota: '剩余配额',
        fieldsRequired: '请同时填写上游网站和邮箱',
        saved: '上游余额查询配置已保存',
        saveFailed: '保存上游余额查询配置失败',
        refreshSuccess: '上游余额已刷新',
        refreshFailed: '刷新上游余额失败',
        status: {
          ok: '查询成功',
          failed: '查询失败',
          unsupported: '站点不支持此登录接口',
          notConfigured: '未配置'
        },
        errors: {
          login_request_failed: '登录请求失败',
          login_rejected: '上游登录被拒绝',
          login_response_invalid: '登录响应未返回可用令牌',
          balance_request_failed: '余额请求失败',
          balance_request_rejected: '余额接口拒绝请求',
          balance_fields_not_found: '响应中未找到余额字段',
          login_endpoint_unsupported: '登录接口不支持',
          balance_endpoint_unsupported: '余额接口不支持'
        }
      },
